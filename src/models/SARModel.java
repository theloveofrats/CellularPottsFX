...
public class SARModel extends BiochemicalModel {
    ...
    public double GetAttackBias(Point point) {
        ...
        double bias = 1+scar_bias*GetLocalConcentration("Actin*", point, "ARITHMETIC");
    ...
    public void InitialiseConqueredPoint(Point target, Point source) {
        ...
        double act   = a_act*actin_max*(1+0.1*ascar*ascar);
        ...
        modelGrid.get("Scar*")[target.x][target.y] = 0.0*dscrS;
        modelGrid.get("Scar")[target.x][target.y] = 0.1*dscr;
        modelGrid.get("Rac")[target.x][target.y] = 0.9*rac1;

        modelGrid.get("Scar*")[source.x][source.y] = 0.0*dscrS;
    ...
    public void Update(Cell c) {
        ...
        for(Point p : c.points){
            double localActin = GetLocalConcentration("Actin*", p, "ARITHMETIC")/actin_max;
            if(localActin<0.5*actin_basal)        localActin = 0;
            ascar[p.x][p.y] +=  Math.min(scar[p.x][p.y], localActin*localActin*localActin)
        ...
        for(Point p : c.points) {
            ...
            double localRac = GetLocalConcentration("Rac", p, "ARITHMETIC");


            double localRecruitment = scar_b * Math.min(currentScarP, localRac*localRac*localRac*localRac);
        ...
        for(Point p : c.points){
            ...
            rac[p.x][p.y] +=    rac_b*(1+0.01*(p.x-mu_x))*GetLocalConcentration("Actin*", p, "ARITHMETIC")
                    - rac_d*rac[p.x][p.y];
